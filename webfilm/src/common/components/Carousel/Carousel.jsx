import React from 'react'

export default function Carousel() {
    return (
        <div id="demo" className="carousel slide" data-ride="carousel">
            {/* Indicators */}
            <ul className="carousel-indicators">
                <li data-target="#demo" data-slide-to={0} className="active" />
                <li data-target="#demo" data-slide-to={1} className />
                <li data-target="#demo" data-slide-to={2} className />
                <li data-target="#demo" data-slide-to={3} className />
                <li data-target="#demo" data-slide-to={4} className />
            </ul>
            {/* The slideshow */}
            <div className="carousel-inner">
                <div className="carousel-item active">
                    <img src="./img/bia.jpg" alt="Los Angeles" width={'100%'} height={600} />
                </div>
                <div className="carousel-item">
                    <img src="./img/bia.jpg" alt="Chicago" width={'100%'} height={600} />
                </div>
                <div className="carousel-item">
                    <img src="./img/bia.jpg" alt="New York" width={'100%'} height={600} />
                </div>
                <div className="carousel-item">
                    <img src="./img/bia.jpg" alt="New York" width={'100%'} height={600} />
                </div>
                <div className="carousel-item">
                    <img src="./img/bia.jpg" alt="New York" width={'100%'} height={600} />
                </div>
            </div>
            {/* Left and right controls */}
            <a className="carousel-control-prev" href="#demo" data-slide="prev">
                <span className="carousel-control-prev-icon" />
            </a>
            <a className="carousel-control-next" href="#demo" data-slide="next">
                <span className="carousel-control-next-icon" />
            </a>
        </div>

    )
}
